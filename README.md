...
- 代理网关：每个运行实例使用独立本地 SOCKS5 网关，支持运行中热切换代理；新连接立即切换，旧连接可自然排空或强制断开
- 分流规则：支持精确域名、域名后缀、域名关键词和 IP/CIDR，并可分别选择 PROXY、DIRECT、BLOCK；代理失败时不会回退直连
- 公共 API：新增代理网关状态、热切换和分流配置接口，并在应用内提供参数、请求示例、响应和错误码说明
...
- 代理热切换与分流：运行中的实例可切换代理，并按域名或 IP 设置代理、直连、阻断规则
