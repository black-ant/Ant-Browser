...
内核管理支持标记内核的**后端类型**，不同后端的可执行文件名、版本号来源和指纹参数集都不一样：

| 后端 | 说明 | 目录要求 | 版本号来源 |
| --- | --- | --- | --- |
| `fingerprint-chromium` | 默认后端，历史内核均按此处理 | 目录下存在 `chrome.exe` | `manifest.json` 或 `*.manifest` |
| `Cloak` | [CloakBrowser](https://github.com/CloakHQ/cloakbrowser) 源码级 patch 内核 | 目录下存在 `chromium-<版本>/` 子目录 | `chromium-<版本>` 目录名 |
...
2. 优先使用应用内下载功能准备内核
3. 如果手动准备内核，请确保目录符合对应后端的要求：`fingerprint-chromium` 需要 `chrome.exe`，`Cloak` 需要 `chromium-<版本>/` 子目录
...
| 管理浏览器内核 | `内核管理` | 新增、编辑、删除、设置默认内核和标记内核后端 |
...
先检查浏览器内核路径是否有效，并确认目录符合所选后端的要求：`fingerprint-chromium` 后端需要目录下存在 `chrome.exe`，`Cloak` 后端需要 `chromium-<版本>/` 子目录。若内核后端标记错了，路径校验也会提示“未找到可执行文件”。
