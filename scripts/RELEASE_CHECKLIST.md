...
- [ ] **创建窗口**：新建一个窗口，列表出现、可编辑。
- [ ] **启动窗口**：点击启动 → 状态 starting→running，浏览器窗口打开。
- [ ] **停止窗口**：点击停止 → 状态回到 stopped，进程关闭。
...
- [ ] **Cookie**：从运行中窗口读取 Cookie → 保存 → 回写，无报错。
- [ ] **扩展绑定**：给窗口绑定一个本地扩展 → 启动浏览器，扩展实际加载（`--load-extension` 生效）。
