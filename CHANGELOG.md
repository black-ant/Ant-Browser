...
- 代理网关：每个运行实例使用独立本地 SOCKS5 网关，支持运行中热切换代理；新连接立即使用新路由，旧连接可自然排空或强制断开。
- 分流规则：支持精确域名、域名后缀、域名关键词和 IP/CIDR，并可分别执行 PROXY、DIRECT、BLOCK；代理失败时不会回退直连。
- 公共 API：新增代理网关状态、热切换和分流配置接口，并补齐参数、请求示例、响应及错误码文档。
